...
export function TodaysMission() {
  ...
  // Generate targets
  const highPriorityTasks = tasks.filter(t => !t.completed && t.priority === "high").slice(0, 2);
  const weakTopics = subjects.flatMap(s => s.chapters.flatMap(c => c.topics)).filter(t => t.status === "weak").slice(0, 2);
  const activeChallenges = challenges.filter(c => !c.completedDays.includes(todayStr)).slice(0, 1);
  const todaysFocus = pomodoroSessions.filter(s => s.completedAt.startsWith(todayStr) && s.type === "focus").length;
  const pomodoroGoal = Math.max(4, todaysFocus + 2); // Dynamic goal
  ...
  return (
    <div className="relative animate-in fade-in slide-in-from-top-4 duration-500 mb-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {highPriorityTasks.length > 0 && (
              <div className="space-y-2">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1"><Brain className="h-3 w-3" /> Priority Tasks</p>
            ...
            {(weakTopics.length > 0 || activeChallenges.length > 0) && (
              <div className="space-y-2">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1"><Flame className="h-3 w-3" /> Revision & Challenges</p>
                <ul className="space-y-1.5">
                  {weakTopics.map(t => (
                    <li key={t.id} className="text-sm flex items-start gap-2">
